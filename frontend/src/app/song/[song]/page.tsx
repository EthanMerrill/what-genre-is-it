...
export default function SongPage({params}: {params: Promise<{song: string}>}) {
	...
	useEffect(() => {
		getTrackData(access_token, searchedSongId).then((data) => {
			setTrackData(data);
		});
	...
	useEffect(() => {
		if (trackData) {
			getArtistData(access_token, trackData?.artists[0].id).then((data: any) => {
	...
	return (
		<div className="h-[70%] w-full">
			<div className="my-auto flex flex-row h-full items-center justify-around max-w-6xl flex-wrap text-slate-50 mx-auto">
				...
				{trackData && (
					<>
						<SongDetails song={trackData?.name} artist={trackData?.artists[0].name} album={trackData?.album.name} genres={genres} />
